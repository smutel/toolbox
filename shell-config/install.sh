...
if [[ "$ID" == "ubuntu" ]]; then
  ...
  wget -O- https://apt.releases.commonfate.io/gpg | sudo gpg --dearmor \
    -o /usr/share/keyrings/common-fate-linux.gpg
  ...
  sudo apt install -y build-essential cmake python3-dev python2.7-dev vim \
    ...
    makepasswd direnv granted > /dev/null
fi
...
mkdir -p ~/.vim/spell
curl -Ls http://ftp.vim.org/pub/vim/runtime/spell/fr.utf-8.spl \
  -o ~/.vim/spell/fr.utf-8.spl
curl -Ls http://ftp.vim.org/pub/vim/runtime/spell/fr.utf-8.sug \
...
"$HOME"/bin/kubectl completion bash | sudo tee /etc/bash_completion.d/kubectl
